$(document).on('click', '#parar-de-seguir', pararDeSeguir);
$(document).on('click', '#seguir', seguir);
$(document).on('submit', '#editar-usuario', editar);
$(document).on('submit', '#atualizar-senha', atualizarSenha);
$(document).on('click', '#deletar-usuario', deletarUsuario);
...
function editar(evento) {
    ...
    $.ajax({
        ...
        Swal.fire("Sucesso!", "Usuário atualiza com sucesso", "success")
        .then(function(){
...
function atualizarSenha(evento) {
    ...
    if ($('#nova-senha').val() != $('#confirmar-senha').val()) {
        Swal.fire("Ops...","A senhas não coincidem","warning");
        return;
    }
    ...
    $.ajax({
        ...
        Swal.fire("Sucesso","A senhas foi atualizada","success")
        .then(function() {
            window.location = "/perfil";
        })
    }).fail(function() {
    ...
    Swal.fire({
        ...
        if (confirmacao.value) {
            $.ajax({
                ...
                Swal.fire("Sucesso!","O seu usuário foi excluído com sucesso!","success")
                .then(function() {
